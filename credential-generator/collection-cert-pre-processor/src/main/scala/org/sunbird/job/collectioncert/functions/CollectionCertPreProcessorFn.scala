    ...
    override def processElement(event: Event,
        ...
        try {
            ...
            if(event.isValid()(config)) {
              ...
              val courseCategory = courseInfo.getOrDefault(config.courseCategory, "").asInstanceOf[String]
              if (config.allowedCourseCategoryForCertificteProcesser.contains(courseCategory)) {
                logger.info(s"Course Category is allowed for certificate Generation :$courseCategory and courseId : ${event.courseId}")
                ...
                logger.info(s"Certificate is generated by dynamicCertificateGeneration for courseId :${event.batchId} and courseCategory $courseCategory and skipped by flink Job.")
              }
